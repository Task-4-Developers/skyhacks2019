...
def generate_json(flat_name):
    ...
    flat_path="main_task/input/%s/"%flat_name
    flat_rooms=[]
    ...
    with open("output_package_%s.json"%flat_name,"w") as output:
        ...
        for image_filename in flat_rooms:
            ...
            #windows
            image_path=os.path.abspath("main_task/input/%s/%s"%(flat_name,image_filename))
            # image_path = "/main_task/input/%s/%s"%(flat_name,image_filename)
            #json input file
